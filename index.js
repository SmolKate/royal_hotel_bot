...
const welcomeMenu = {
    reply_markup: {
        keyboard: [
            ...
            [{ text: 'Как заказать экскурсию'}, { text: 'Популярные рестораны города'}],
...
const spaMenu = [
    {title: 'Как записаться в СПА?', message: 'Вы можете позвонить из номера (2060), по телефону +7 (4852) 67-29-09 или пройти на стойку ресепшен СПА-центра.'},
    {title: 'Где расположен СПА-центр?', message: 'СПА-центр расположен на 0 этаже отеля. При выходе из лифта направлятесь на право.'},
...
const breakfastMenu = [
    ...
    {title: 'Завтрак в лобби баре', message: 'Вы можете пройти в Лобби - бар, который расположен на  0 этаже отеля Royal SPA. Для наших гостей разработано меню ранних завтраков, которое действует ежедневно с 05:00 до 12:00. Забронировать стол можно по номеру (2002).'},
    {title: 'Завтрак шведский стол', message: 'Вы можете пройти в Лобби - бар, который расположен на  0 этаже отеля Royal SPA. Для наших гостей разработано меню ранних завтраков, которое действует ежедневно с 05:00 до 12:00. Забронировать стол можно по номеру (2002).'},
    {title: 'Назад'}
]

const wifiMenu = [
    {title: 'Wi-Fi', message: `*Подключиться к сети Wi\\-Fi\\.*\n\nCеть: ROYAL HOTEL SPA\\.\nПароль: регистарция по Вашему личному номеру телефона`},
    {title: 'Бизнес центр', message: `На 0 этаже отеля расположен бизнес\\-центр с компьютером и принтером, которыми Вы также можете воспользоваться в любое время\\. Пароль для входа: 123`},
    {title: 'Назад'}
]
...
const start = () => {
    ...
    bot.on('message', async (msg) => {   // request on message receive
        ...
        if (text === 'Утюг и гладильная доска') {
        ...
        if (text === 'Популярные рестораны города') {
            back.push(text)
            return bot.sendMessage(chatId, `${text}`, menuOptions(restaurantMenu))
        }
